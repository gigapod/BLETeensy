...
public:
    ...
    // Pump BTstack's HCI UART and run loop. BTstackTeensy has no background
    // core/driver of its own (unlike the Pico W's cyw43_arch), so this MUST
    // be called once per Arduino loop() iteration or BLE processing stalls.
    void update();
