...
public:
    ...
    String getString() {
        // No String(const char *, size_t) constructor on Teensyduino (unlike
        // the Pico core), and the underlying value isn't guaranteed
        // null-terminated -- build the String byte by byte instead.
        String s;
        size_t len = valueLen();
        if (len) {
            s.reserve(len);
            const char *d = (const char *)valueData();
