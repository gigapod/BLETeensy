...
// To use:
// Run a copy of CharacteristicOnWrite on another Teensy
// Then run this sketch on a separate Teensy to demonstrate
// 1. Scanning for specific BLE services
// 2. Automatically connecting to the remote Teensy
...
// Wait ms milliseconds, pumping BLE.update() throughout so BTstack (and any
// notifications we've subscribed to) keep being serviced -- see README.md.
void pumpDelay(uint32_t ms) {
...
void loop() {
  ...
  // Print them out to screen (normally only 1 Teensy present)
  int i = 0;
  ...
  if (report->empty()) {
    Serial.println("No Teensy Servers found, retrying in 10 seconds");
    pumpDelay(10000);
    return;
  }
  ...
  // Wait a bit, we'll get notifications every second while we pump BLE.update()
  pumpDelay(10000);
